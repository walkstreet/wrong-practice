...
from app.deps import LearnerOnly, get_db

router = APIRouter(prefix="/api/v1/me", tags=["me-assignments"], dependencies=[LearnerOnly])


@router.get("/assignments", response_model=list[schemas.LearnerAssignmentListItemOut])
def list_my_assignments(db: Session = Depends(get_db), user=LearnerOnly) -> list[schemas.LearnerAssignmentListItemOut]:
