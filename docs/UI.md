...
- 左：`共 **N** 条`（数字用主文字）；超管批量模式下跟「全选」和「已选 M 题」
- 右：超管「批量发布公共库」+ 工具 + 表格/卡片切换（分段控件，同筛选胶囊）
- 默认不出现勾选。点「批量发布公共库」后进入批量模式：出现全选、卡片角标勾选、表格行勾选；表头不要勾选框。已发布题目不可选，全选也跳过它们。完成或取消后退出批量模式。
...
- 批量模式：未发布题在左上圆角挂一枚勾选角标（`.list-qcard-check`），一半在卡外一半在卡内，微旋；勾选不打开详情。已发布的不出现勾选。选中：边 `#7c5cfc`、底 `#f6f3fb`、focus 环
- 顶栏：左难度 + 错误率胶囊，右 `#ID`
...
1. **分段胶囊**：错误率（全部 / 高 / 中 / 低）与难度（全部 / 入门 / 基础 / 中等 / 较难 / 挑战）同一行按内容排列，不够则换行。点即筛。
   - 错误率：高 ≥ 75%（含 100%），中 50%–75%，低 < 50%（含 25%）；未练只出现在「全部」
   - 难度：未评级只出现在「全部」
2. **下一行**：知识点（最宽）→ 题型 → 题目 ID（固定约 176px，回车查找）。
3. 有条件时该行最右侧出现文字链「清除条件」。
...
- `scroll.x` 约 1266（超管批量多选列约 1314），内容超出时横向滚动
- **操作列 `fixed: "right"`**，白底（表头跟表头色）；滚动时左侧内阴影
- 超管仅在批量模式 `rowSelection`；`hideSelectAll`，表头不要勾选；已发布行禁用勾选；选中行底 `#efe8ff`
...
| 字段 | 展示 |
| --- | --- |
| 难度 | 胶囊：1–2 绿、3 紫、4 橙、5 红；未评级灰 |
| 错误率 | 胶囊：高红、中橙、低绿、未练灰；hover 显示百分比 |
| 录入来源 | 手动录入 / 识别录入 |
